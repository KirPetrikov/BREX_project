...
def parse_dfnfnr_genes(
    ...
    df = pd.read_csv(path_to_tsv, sep='\t',
                     ...
                     names=['Nucleotide', 'Protein', 'Annotation', 'tmp_ID',
    ...
    df['Accession'] = path_to_tsv.name[:-10]

    # Add coords for protein genes
    if path_to_gff:
        df_gff = parse_prodigal_gff(path_to_gff)
...
def process_single_dfnfnr_table(
    ...
    """
    df_defsys = parse_dfnfnr_genes(path_to_tsv, path_to_gff)
...
def process_dfnfnr_data(
    ...
    for folder in input_data_path.iterdir():
        ...
        summary_curr, prot_curr, rdn_curr, anti_curr = process_single_dfnfnr_table(
            folder / f'{folder.name}_genes.tsv',
            input_gff_path / f'{folder.name}/{folder.name}_prodigal.gff')
